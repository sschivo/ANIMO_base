...
public class Model implements Serializable {
	 ...
	 * the upper bound is not "too large" for UPPAAL to understand it
	 * @return The intermediate ANIMO model
	 * @throws InatException
	 */
	@SuppressWarnings("unchecked")
	public static Model generateModelFromCurrentNetwork(TaskMonitor monitor, int nMinutesToSimulate) throws InatException {
		...
		for (int i = 0; edges.hasNext(); i++) {
			...
			List<Double> times = scenario.generateTimes(1 + nLevelsR1, activeR1, 1 + nLevelsR2, activeR2);
	 ...
	 */
	@SuppressWarnings("unchecked")
	private static void checkParameters(TaskMonitor monitor, int nMinutesToSimulate) throws InatException {
		final String NUMBER_OF_LEVELS = Model.Properties.NUMBER_OF_LEVELS, //The total number of levels for a node (=reactant), or for the whole network (the name of the property is the same)
		SECONDS_PER_POINT = Model.Properties.SECONDS_PER_POINT, //The number of real-life seconds represented by a single UPPAAL time unit
		SECS_POINT_SCALE_FACTOR = Model.Properties.SECS_POINT_SCALE_FACTOR, //The scale factor for the UPPAAL time settings, allowing to keep the same scenario parameters, while varying the "density" of simulation sample points
		LEVELS_SCALE_FACTOR = Model.Properties.LEVELS_SCALE_FACTOR, //The scale factor used by each reaction to counterbalance the change in number of levels for the reactants.
...
//					/*if (rate > 1) {
//						System.err.println("\tIl rate (" + rate + ") � infatti > 1");
//					} else {
//						System.err.println("\tIl rate (" + rate + ") per� NON � > 1! Il reciproco viene " + (int)Math.round(1 / rate) + ", ma l'uno ce l'abbiamo perch� facciamo -" + uncertainty + "%, che viene appunto " + ((int)((int)Math.round(1 / rate) * (100.0 - uncertainty) / 100.0)));
//					}*/
//					//System.err.println("\tQuindi consiglio di DIVIDERE sec/step almeno per " + (1.5 * rate / ((100.0 - uncertainty) / 100.0)) + ", ottenendo quindi non pi� di " + (secPerStep / (1.5 * rate / (secStepFactor * (100.0 - uncertainty) / 100.0))));
...
//			}
		...
		if (nMinutesToSimulate >= 0) { //If we were requested to perform a simulation (and not simply translate the model "out of curiosity", we were given also a time limit for the simulation. We need to check whether that time limit is a "large number"
